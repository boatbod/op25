...
void bind_rmsagc_ff(py::module &m) {
  ...
  py::class_<rmsagc_ff, gr::sync_block, gr::block, gr::basic_block,
      ...
      .def(py::init(&rmsagc_ff::make), py::arg("alpha") = 0.01,
           py::arg("k") = 1., D(rmsagc_ff, make))

      .def("set_alpha", &rmsagc_ff::set_alpha, py::arg("alpha"),
           D(rmsagc_ff, set_alpha))

      .def("set_k", &rmsagc_ff::set_k, py::arg("k"), D(rmsagc_ff, set_k))
